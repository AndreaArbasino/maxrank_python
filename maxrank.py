...
def genhammingstrings(strlen, weight):
    ...
    The approach used is bottom-up, building strings with a bigger weight from those with a lower one.
    The strings are handled in their binary rep and ultimately converted.
    """
...
def ba_hd(data, p):
    ...
    for leaf in leaves:
        ...
        # hamweight = |F_l|, leaf.order = p_order as described in the Mouratidis paper
        # BA visits the leaves in increasing order of cardinality of |F_l| (see hamweight)
        hamweight = 0
...
def aa_2d(data, p):
    ...
    dominators = query.getdominators(data, p)
    incomp = query.getincomparables(data, p)
    sky = query.getskyline(incomp) #SL

    # In 2D all the half-spaces become half-lines
    p_line = HalfLine(p)

    # "intervals" represents MA: the Mixed Arrangement
    intervals = SortedList(key=lambda el: el.range[1])
    # insert into MA an augmenting half-space for each record in SKY
    for sp in sky:
        sp_line = HalfLine(sp)
        pnt = find_halflines_intersection(p_line, sp_line)
        intervals.add(Interval(sp_line, np.array([np.nan, pnt.coord[0]]), sp_line.q < p_line.q))
    intervals.add(Interval(None, np.array([np.nan, 1]), False))
    print("> {} halflines(s) have been inserted".format(len(sky)))

    n_exp = 0

    while True:
        minorder = np.inf   # minimum order, initially set to +infinity
        mincells = []       # set of cells of minimum order, equivalent to "T" in the paper
        last_end = 0

        covering = [cell.halfline for cell in intervals if cell.coversleft]

        # Identify the cell(s) in MA(intervals) with the minimum order (Line 5)
        ...
        if len(to_expand) == 0:
            ...
            for hl in to_expand:
                hl.arr = Arrangement.SINGULAR       # Mark every half-space h inside to_expand as singular in MA
                incomp.remove(hl.pnt)

            new_sky = query.getskyline(incomp)      # Update SL by ignoring all expanded records
...
def aa_hd(data, p):
    ...
    while True:
        ...
        for leaf in leaves:
            # If the minimum order is greater than o* then break
            ...
            while hamweight <= len(leaf.halfspaces) \
                ...
                if hamweight >= 3:
                    print("> Leaf {}: Evaluating Hamming strings of weight {}".format(id(leaf), hamweight))
                hamstrings = genhammingstrings(len(leaf.halfspaces), hamweight)
                # Identify the cell(s) in MA with the minimum order
                cells = searchmincells_lp(leaf, hamstrings)
